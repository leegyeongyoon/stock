...
import {
  getGyleeTradingStatus,
  startGyleeTrading,
  stopGyleeTrading,
  getGyleePositions,
  type GyleeTradingStatus,
  type HongPosition,
  type HongEvent,
} from "@/lib/api";
...
const STATE_LABELS: Record<string, { label: string; color: string; dotColor: string }> = {
  ...
  SCANNING: { label: "스캔중", color: "text-purple-400", dotColor: "bg-purple-500 animate-pulse" },
...
export default function GyleeTradingPanel() {
  const queryClient = useQueryClient();
  const wsRef = useRef<WebSocket | null>(null);
  const [wsEvents, setWsEvents] = useState<HongEvent[]>([]);

  const { data: status } = useQuery({
    queryKey: ["gylee-trading-status"],
    queryFn: getGyleeTradingStatus,
    refetchInterval: 5000,
    retry: 2,
  });

  const { data: positionsData } = useQuery({
    queryKey: ["gylee-positions"],
    queryFn: getGyleePositions,
    refetchInterval: 5000,
    retry: 2,
  });

  const startMutation = useMutation({
    mutationFn: startGyleeTrading,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["gylee-trading-status"] });
      queryClient.invalidateQueries({ queryKey: ["gylee-filters"] });
    },
  });

  const stopMutation = useMutation({
    mutationFn: stopGyleeTrading,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["gylee-trading-status"] });
    },
  });
    ...
    <div className="bg-slate-800 rounded-xl border border-slate-700 p-4 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <h3 className="text-sm font-semibold text-white">경윤 자동매매</h3>
                ...
                {positions.map((pos) => (
                  <tr key={pos.stock_code} className="border-b border-slate-700/50 hover:bg-slate-700/30">
                    ...
                    <td className="text-right text-slate-300">
                      {pos.quantity}
                      {pos.partial_sold && (
                        <span className="text-[9px] text-yellow-400 ml-1">/{pos.original_quantity}</span>
                      )}
                    </td>
                    <td className="text-right text-slate-300 font-mono">{formatPrice(pos.avg_price)}</td>
                    <td className="text-right text-slate-300 font-mono">{formatPrice(pos.current_price)}</td>
                    ...
                    <td className="text-center">
                      {pos.partial_sold ? (
                        <span className="text-[10px] px-1.5 py-0.5 rounded bg-yellow-500/20 text-yellow-400">반익절</span>
                      ) : (
                        <span className="text-[10px] px-1.5 py-0.5 rounded bg-purple-500/20 text-purple-400">보유</span>
                      )}
                    </td>
                  </tr>
                ))}
