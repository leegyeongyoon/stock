...
# v6 점수 기반 선별 파라미터
MIN_QUALITY_SCORE = 60   # 최소 품질 점수 (100점 만점) - v6.2: 50~59점 제외
QUALITY_WEIGHT = {
...
def simulate_day(bars_all, daily_info, date, prev_date, capital, positions, overnight_stats, quality_stats):
    ...
    for e in entries:
        ...
        # v6.1: 70~79점 구간 스킵 (일관된 손실 구간)
        if 70 <= quality_score < 80:
            skipped_reasons["70-79점스킵"] += 1
            quality_stats["filtered_70_79"] += 1
            continue
...
def main():
    ...
    dates_all = get_trading_days(END_DATE, 60)
